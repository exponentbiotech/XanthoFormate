...
def plot_market_viability_overview(evaluations: Iterable[ScenarioEvaluation]) -> plt.Figure:
    ...
    for ax, cat in zip(axes, categories):
        ...
        ax.set_title(CAT_LABEL[cat], pad=12)
...
def plot_cost_structure(evaluations: Iterable[ScenarioEvaluation],
    ...
    for ax, row in zip(axes, subset):
        ...
        for group_label, keys in COST_GROUPS.items():
            ...
            if abs(total) > 1e-6:
                grouped[group_label] = total

        scp_credit = row.tea.credits_usd_per_y.get("scp_credit", 0.0) / kg
        struvite_credit = row.tea.credits_usd_per_y.get("struvite_credit", 0.0) / kg
        other_credits = sum(
            v for k, v in row.tea.credits_usd_per_y.items()
            if k not in ("scp_credit", "struvite_credit")
        ) / kg

        labels = list(grouped.keys())
        values = list(grouped.values())
        if struvite_credit > 1e-6:
            labels.append("Struvite credit")
            values.append(-struvite_credit)
        if scp_credit > 1e-6:
            labels.append("SCP credit")
            values.append(-scp_credit)
        if other_credits > 1e-6:
            labels.append("Other credits")
            values.append(-other_credits)

        # Reverse order so cost bars build bottom-to-top (electricity at bottom,
        # credits visually "cancelling" at top) without needing invert_yaxis().
        labels = labels[::-1]
        values = values[::-1]

        y_pos = np.arange(len(labels))
        cost_idx = 0  # separate palette index for cost bars only
        colors = []
        for v in values:
            if v < 0:
        ...
        for bar, val in zip(bars, values):
            is_credit = val < 0
            if is_credit:
                # Place credit labels just left of zero so they're always readable
                ...
                ax.text(val + 0.08, bar.get_y() + bar.get_height() / 2,
                        _usd(val), ha="left", va="center", fontsize=8,
        ...
        # Expand xlim so right-side labels don't clip
        xlo, xhi = ax.get_xlim()
        pad_r = (xhi - xlo) * 0.12
        pad_l = (xhi - xlo) * 0.04
        ax.set_xlim(xlo - pad_l, xhi + pad_r)

        ax.text(gross, len(labels) - 0.6, f"Gross {_usd(gross)}", fontsize=7.5,
                ha="center", color="#666666")
        ax.text(net, len(labels) + 0.25, f"Net {_usd(net)}", fontsize=8.5,
                ha="center", color=CREDIT_COLOR, fontweight="600")

        ax.axvline(0, color="#555555", lw=0.6)
        ax.set_yticks(y_pos)
        ax.set_yticklabels(labels, fontsize=9.5)
        ax.set_xlabel("USD / kg primary product")
        ax.set_title(f"{CAT_LABEL[row.foreground.scenario.category]}  "
...
def plot_margin_curve(evaluations: Iterable[ScenarioEvaluation]) -> plt.Figure:
    ...
    for ax, cat in zip(axes, [ScenarioCategory.AMMONIA_SCP, ScenarioCategory.BIO_UREA_SCP]):
        ...
        ax.set_ylabel("USD / kg primary product")
        ax.set_title(f"Economies of Scale \u2014 {CAT_LABEL[cat]}", pad=12)
        ax.legend(fontsize=7.5, loc="upper right")
...
def plot_sensitivity_tornado(config: ScenarioConfig,
    ...
    ax.set_title(
        ...
        f"{CAT_LABEL[config.category]} at {int(config.annual_primary_product_tpy):,} t/y",
        pad=14)
...
def plot_executive_summary(evaluations: Iterable[ScenarioEvaluation]) -> plt.Figure:
    ...
    for cat in [ScenarioCategory.AMMONIA_SCP, ScenarioCategory.BIO_UREA_SCP]:
        ...
        ax_gwp.plot(caps, gwp, marker="s", markersize=7, linewidth=2.2,
                    color=CAT_COLOR[cat], label=f"{CAT_LABEL[cat]} (current settings)")
    ...
    try:
        ...
        for cat in [ScenarioCategory.AMMONIA_SCP, ScenarioCategory.BIO_UREA_SCP]:
            ...
            if fav_sub:
                ...
                ax_gwp.plot(fcaps, fgwp, marker="^", markersize=6, linewidth=1.6,
                            linestyle="--", color=CAT_COLOR[cat], alpha=0.6,
                            label=f"{CAT_LABEL[cat]} (renew. + biogenic)")
    except Exception:
